...
const CustomerLogin = () => {
  ...
  const { setCustomer, setAdmin, setToken } = useAuth();
  ...
  const handleLogin = async (e) => {
    ...
    try {
      // 1️⃣ Attempt ADMIN Login FIRST
      try {
        const adminRes = await axios.post(`${API_BASE_URL}/admin/Login`, {
        ...
        localStorage.setItem("adminAuthData", JSON.stringify(user));
        localStorage.setItem("userType", "admin");

        // ✅ Remove any other user sessions
        ...
        return; // ⛔ Stop — do NOT try customer login
      } catch (adminErr) {
        // Admin login failed → continue to customer login
      }
      ...
      const { token, user } = customerRes.data;
